...
import (
	...
	"github.com/butler/butler/apps/tool-webfetch/internal/runtime"
...
type App struct {
	config     config.ToolWebFetchConfig
	log        *slog.Logger
...
func New(context.Context) (*App, error) {
	cfg, _, err := config.LoadToolWebFetchFromEnv()
	if err != nil {
		return nil, err
	}
	...
	runtimeServer := runtime.NewServer(runtime.NewProviderChain(
		runtime.NewSelfHostedProvider(cfg.SelfHostedBaseURL, httpClient),
		runtime.NewJinaReaderProvider(cfg.JinaBaseURL, cfg.JinaAuthToken, httpClient),
		runtime.NewPlainHTTPProvider(httpClient, cfg.PlainHTTPEnabled),
	), log)
	...
	httpMux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) { _, _ = w.Write([]byte("tool-webfetch")) })
...
func (a *App) Run(ctx context.Context) error {
	...
	go func() {
		a.log.Info("starting tool webfetch server", slog.String("http_addr", a.config.Shared.HTTPAddr))
	...
	go func() {
		a.log.Info("starting tool webfetch grpc server", slog.String("grpc_addr", a.config.Shared.GRPCAddr))
...
func (a *App) shutdown(runErr error) error {
	...
	if runErr != nil {
		a.log.Error("tool webfetch stopped with error", slog.String("error", runErr.Error()))
		return runErr
	}
	return nil
}
