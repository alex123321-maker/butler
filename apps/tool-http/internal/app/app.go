...
import (
	...
	"github.com/butler/butler/apps/tool-http/internal/runtime"
...
func New(context.Context) (*App, error) {
	cfg, _, err := config.LoadToolHTTPFromEnv()
	if err != nil {
		return nil, err
	}
	...
	runtimev1.RegisterToolRuntimeServiceServer(grpcServer, runtime.NewServer(nil, log))

	httpMux := http.NewServeMux()
	httpMux.Handle("/health", health.New(cfg.Shared.ServiceName).Handler())
	httpMux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) { _, _ = w.Write([]byte("tool-http")) })
...
func (a *App) Run(ctx context.Context) error {
	...
	go func() {
		a.log.Info("starting tool http server", slog.String("http_addr", a.config.Shared.HTTPAddr))
	...
	go func() {
		a.log.Info("starting tool http grpc server", slog.String("grpc_addr", a.config.Shared.GRPCAddr))
...
func (a *App) shutdown(runErr error) error {
	...
	if runErr != nil {
		a.log.Error("tool http stopped with error", slog.String("error", runErr.Error()))
		return runErr
	}
	return nil
}
