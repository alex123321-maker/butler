...
import (
	...
	"github.com/butler/butler/apps/tool-browser/internal/runtime"
...
type App struct {
	config     config.ToolBrowserConfig
	log        *slog.Logger
...
func New(context.Context) (*App, error) {
	cfg, _, err := config.LoadToolBrowserFromEnv()
	if err != nil {
		return nil, err
	}
	...
	httpMux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) { _, _ = w.Write([]byte("tool-browser")) })
...
func (a *App) Run(ctx context.Context) error {
	...
	go func() {
		a.log.Info("starting tool browser server", slog.String("http_addr", a.config.Shared.HTTPAddr))
	...
	go func() {
		a.log.Info("starting tool browser grpc server", slog.String("grpc_addr", a.config.Shared.GRPCAddr))
...
func (a *App) shutdown(runErr error) error {
	...
	if runErr != nil {
		a.log.Error("tool browser stopped with error", slog.String("error", runErr.Error()))
		return runErr
	}
	return nil
}
