...
import (
	...
	"github.com/butler/butler/apps/tool-browser-local/internal/runtime"
...
type App struct {
	config     config.ToolBrowserLocalConfig
	log        *slog.Logger
...
func New(context.Context) (*App, error) {
	cfg, _, err := config.LoadToolBrowserLocalFromEnv()
	if err != nil {
		return nil, err
	}
	...
	orchestratorClient := client.New(cfg.OrchestratorBaseURL, time.Duration(cfg.RequestTimeout)*time.Second, nil)
	var actionDispatcher interface {
		DispatchAction(ctx context.Context, params client.DispatchActionParams) (client.DispatchActionEnvelope, error)
	}
	dispatchMode := resolveDispatchMode(cfg.DispatchRolloutMode, cfg.DispatchMode)
	switch dispatchMode {
	case "orchestrator_relay":
		actionDispatcher = client.NewOrchestratorRelay(cfg.OrchestratorBaseURL, time.Duration(cfg.RequestTimeout)*time.Second, nil)
	default:
		actionDispatcher = client.NewBrowserBridge(cfg.BrowserBridgeURL, time.Duration(cfg.RequestTimeout)*time.Second, nil)
	}
	runtimeServer := runtime.NewServer(orchestratorClient, actionDispatcher, log)
	runtimev1.RegisterToolRuntimeServiceServer(grpcServer, runtimeServer)

	httpMux := http.NewServeMux()
	httpMux.Handle("/health", health.New(cfg.Shared.ServiceName).Handler())
	httpMux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) { _, _ = w.Write([]byte("tool-browser-local")) })
...
func (a *App) Run(ctx context.Context) error {
	...
	go func() {
		a.log.Info("starting tool browser local server", slog.String("http_addr", a.config.Shared.HTTPAddr))
	...
	go func() {
		a.log.Info("starting tool browser local grpc server", slog.String("grpc_addr", a.config.Shared.GRPCAddr))
...
func (a *App) shutdown(runErr error) error {
	...
	if runErr != nil {
		a.log.Error("tool browser local stopped with error", slog.String("error", runErr.Error()))
		return runErr
	}
	return nil
}
...
func resolveDispatchMode(rolloutMode, explicitMode string) string {
	switch rolloutMode {
	case "remote_preferred":
		return "orchestrator_relay"
	case "dual":
		if explicitMode == "orchestrator_relay" {
			return explicitMode
		}
