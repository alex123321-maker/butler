...
func TestResolveToolCall(t *testing.T) {
	...
	broker := NewToolCallBroker(
		NewBroker(stubMetadataStore{record: Record{Alias: "github", Status: StatusActive, AllowedDomains: []string{"api.github.com"}, AllowedTools: []string{"http.request"}, ApprovalPolicy: ApprovalPolicyAutoReadOnly, SecretRef: "env://GITHUB_TOKEN"}}),
